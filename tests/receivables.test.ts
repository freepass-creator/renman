...
describe('앵커 load-bearing 가드 — 스케줄로 표현 못 하는 carry도 net에 보존', () => {
  // 이 케이스들은 _carryUnpaid 앵커(seedNet 특례)가 지탱한다. schedGross로만 도출하면 0으로 유실됨.
  // (opening-balance 단일화 시도 때 실데이터 13계약·17.77M 유실 → 특례가 안전망임을 확인, 여기에 박제)
  it('start·contractDate 결손 씨앗 → net == carry (회차 못 만들어도)', () => {
    expect(net({ startDate: '', contractDate: '', endDate: '', _carryUnpaid: 1_000_000 })).toBe(1_000_000);
  });
