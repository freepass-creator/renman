...
export function useEntityLists(keys: readonly string[]): {
  data: EntityRecord[][];
  loading: boolean;
  reload: () => void;
} {
  ...
  const ks = useMemo(() => keyStr.split(','), [keyStr]);
...
export function useEntityList(key: string): { rows: EntityRecord[]; loading: boolean; reload: () => void } {
  const keys = useMemo(() => [key], [key]);
  const { data, loading, reload } = useEntityLists(keys);
