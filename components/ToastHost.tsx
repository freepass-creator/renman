...
const STYLE: Record<T['kind'], { bg: string; fg: string; Icon: LucideIcon }> = {
  success: { bg: '#132a1e', fg: '#d1fae5', Icon: CheckCircle },
  error: { bg: '#3a1418', fg: '#fecaca', Icon: AlertCircle },
  info: { bg: '#1e293b', fg: '#e2e8f0', Icon: Info },
};
      ...
      {toasts.map((t) => {
        ...
        return (
          <div key={t.id} onClick={() => dismiss(t.id)} role="status"
            ...
            <span style={{ flexShrink: 0, width: 18, height: 18, borderRadius: '50%', background: 'rgba(255,255,255,0.16)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }}>
