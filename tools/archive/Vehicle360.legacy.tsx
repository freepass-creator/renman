...
import { Sec, Cards, Metric, ObjCard, Btn, TextLink, Badge, KV, HiddenSecs, EmptyState, Message, Input, Select, SectionLabel, Disclosure, th, thR, td, tdR, won, C, SH, PageLoading, useConfirm, type Step, type KVRow } from '@/components/ui';
...
// 자산상세 = 통상적 세부페이지 원자 배열(SSOT).
//   껍데기 DetailShell → 신분 → 미결칩 → CTA → 본문 4장(SectionLabel)+Sec 스택.
//   장: 지금 · 이 차 · 수납·정산 · 이력. 계약조건=현황 하위 · 할부표=취득 Disclosure.
//   스펙 캔버스: vehicle360-detail-spec. 드래그 순서는 useSecOrder가 이 기본순 위에 저장.
const SEC_DEFAULT = [
  'v-status', 'v-gps',
  'v-info', 'v-reg', 'v-insurance', 'v-purchase', 'v-product', 'v-econ',
  'v-schedule', 'v-deposit', 'v-handover',
  'v-penalty', 'v-work', 'v-history',
];
const GROUP_NOW = ['v-status', 'v-gps'] as const;
const GROUP_CAR = ['v-info', 'v-reg', 'v-insurance', 'v-purchase', 'v-product', 'v-econ'] as const;
const GROUP_CASH = ['v-schedule', 'v-deposit', 'v-handover'] as const;
const GROUP_LOG = ['v-penalty', 'v-work', 'v-history'] as const;
...
export function Vehicle360({ plate, focus }: { plate: string; focus?: string }) {
  ...
  const [loanOpen, setLoanOpen] = useState(false); // 취득 Sec 안 할부스케줄 인라인 펼침
  ...
  const groupOrd = (ids: readonly string[]) => Math.min(...ids.map((id) => { const i = secOrd(id); return i < 0 ? 999 : i; }));
  ...
  useEffect(() => {
    ...
    if (focus === 'insurance') { window.setTimeout(() => document.getElementById('v-insurance')?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 80); }
  }, [focus]);
  ...
  return (
    <div>
      ...
      <div style={{ order: groupOrd(GROUP_NOW) }}><SectionLabel mt={4}>지금</SectionLabel></div>
      <div style={{ order: groupOrd(GROUP_CAR) }}><SectionLabel>이 차</SectionLabel></div>
      <div style={{ order: groupOrd(GROUP_CASH) }}><SectionLabel>수납 · 정산</SectionLabel></div>
      <div style={{ order: groupOrd(GROUP_LOG) }}><SectionLabel>이력</SectionLabel></div>
      ...
      {/* 현황 = 한눈 + (계약 있으면) 계약조건 하위. 계약조건은 별도 Sec 아님. */}
      <Sec id="v-status" title="현황" desc="한눈 요약 · 계약 조건" order={secOrd('v-status')} onReorder={reorderSec}
        right={active
      ...
      {/* 취득 · 구입 — 어떻게 샀는지. 할부면 Disclosure로 스케줄 인라인 펼침(별도 Sec 아님). */}
      {(v || editInfo) ? <Sec id="v-purchase" order={secOrd('v-purchase')} onReorder={reorderSec} title="취득 · 구입" desc="소비자가·개소세 · 취득·매입 · 할부" tone={editInfo ? 'ok' : undefined} right={
        editInfo
          ? <span style={{ fontSize: 11.5, color: C.faint }}>함께 편집 중</span>
      ...
      {/* 수납 스케줄 — 현재 계약 회차 + 이전 계약 수납 요약(현재 vs 이전). */}
      {(active || pastContracts.length > 0) ? <Sec id="v-schedule" order={secOrd('v-schedule')} onReorder={reorderSec} title="수납 스케줄" n={active ? schedule.length : pastContracts.length} desc={active ? '회차별 청구·미납 · 미수관리' : '이전 계약 수납 이력'}
