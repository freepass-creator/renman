...
(async () => {
  ...
  await p.evaluate(() => window.dispatchEvent(new CustomEvent('jpk:print-doc', { detail: { type: 'contract', plate: '365주3303' } })));
  await p.waitForTimeout(2000);
  await p.screenshot({ path: 'C:/dev/jpkerp6-app/docs/contract.png', fullPage: true }); console.log('contract done');
  await b.close();
