...
import { ALL_COMPANIES, COMPANY_DEFS, RENTAL_COMPANY_IDS, companyLabel } from '@/lib/companies';
...
const NAV: Array<{ key: Workspace; label: string; href: string }> = [
  { key: 'work', label: '업무조회', href: '/sheet/reborn' },
  ...
  return <header className={styles.header}>
    <div className={styles.identity}>
      ...
      <div className={styles.companySelect}>
        <label htmlFor={`reborn-company-${active}`}>관리 회사</label>
        <SheetSelect id={`reborn-company-${active}`} value={companyId} onChange={(event) => setCompanyId(event.target.value)} disabled={!isOperator}>
    ...
    <div className={styles.user} title={`${userName} · ${user?.email || ''}`}>{userName.slice(0, 1)}</div>
  </header>;
}
