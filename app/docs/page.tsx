...
function reprint(d: Doc) {
  ...
  if (!w) return;
  w.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>${d.docNo || ''}</title><style>@page{size:A4;margin:0}body{margin:0;background:#fff}${DOC_PRINT_CSS}</style></head><body><div class="doc-paper" style="position:relative">${d.docNo ? `<div class="doc-no">${d.docNo}</div>` : ''}${d.bodyHtml || ''}</div><script>window.onload=function(){setTimeout(function(){window.print()},250)}<\/script></body></html>`);
  w.document.close();
}
