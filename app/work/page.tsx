...
import {
  Badge, Btn, C, LedgerCreatePanel, LedgerFrame, LedgerRecordPanel, PillTabs, Search, won,
...
type WorkLedgerRow = {
  ...
  assignee: string;
  amount: number;
  source: 'work_item' | 'history' | 'penalty' | 'inbox';
  raw: EntityRecord;
};
...
const BASIC_COLS: SheetCol<WorkLedgerRow>[] = [
...
const ALL_COLS: SheetCol<WorkLedgerRow>[] = [
  ...BASIC_COLS,
  ...
  { key: 'source', label: '원천', render: (r) => r.source, text: (r) => r.source },
];
...
export default function WorkLedgerPage() {
  ...
  ), [allRows, group, assignee, range.from, range.to, q]);
  ...
  return (
    <LedgerFrame
      ...
      right={<span style={{ display: 'inline-flex', gap: 6 }}>
        <Btn size="sm" variant={creating ? 'ghost' : 'solid'} aria-pressed={creating} onClick={() => {
          setSelected(null);
          setCreating((open) => !open);
        }}><Plus size={14} /> {creating ? '생성 취소' : '업무 생성'}</Btn>
        <Btn size="sm" variant="ghost" href="/penalty/upload"><UploadCloud size={14} /> 과태료 OCR</Btn>
      </span>}
      filters={<>
