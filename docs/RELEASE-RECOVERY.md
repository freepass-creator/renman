...
- 배포 직후 `npm run test:production`을 실행한다. 운영 33개 경로, `/api/health`의 production·commit 식별, 보안 헤더, 미인증 API 차단(401/403)을 한 번에 확인한다.
- 기본 브랜치의 `.github/workflows/production-smoke.yml`은 매일 09:10 KST에 같은 검사를 실행하며 Actions에서 수동 실행할 수도 있다.
- 스모크 실패 시 추가 배포를 중단하고 `/api/health`의 commit과 Vercel의 현재 Production 배포 ID를 먼저 대조한다.
- 경로 5xx 또는 health 실패면 직전 Ready 배포로 롤백한 뒤 스모크를 다시 실행한다. 인증 경계 실패(보호 API 2xx)는 데이터 노출 위험이므로 즉시 출시 차단으로 처리한다.
- 롤백 후에도 실패하면 Firebase 상태와 최근 Rules 배포를 확인한다. 데이터 복구는 화면 오류만으로 실행하지 않고, 실제 데이터 손상 및 백업 메타데이터 확인 후 위 복구 절차에 따라 별도 승인한다.
