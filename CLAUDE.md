...
### 업무관리의 정의 (사장님 확정 2026-08-06) — 「할 일이 생성되는 곳」

**업무관리(`/work`)는 사건 보관소가 아니라 «하는 일»이 만들어지는 곳이다.**
지나간 일을 적어두는 자리가 아니라, **앞으로 누가 무엇을 할지가 생기는 자리**다.

판정 기준 — 업무인지 아닌지는 이 셋이 붙는가로 가른다:

| 붙는 것 | 뜻 | 없으면 |
|---|---|---|
| **담당** | 누가 하는가 | 아무도 안 한다 → 업무 아님 |
| **기한** | 언제까지 | 안 끝난다 → 업무 아님 |
| **상태** | 어디까지 됐나 | 끝난 줄 모른다 → 업무 아님 |

셋이 안 붙는 것은 업무가 아니라 **원장의 속성**이거나 **이력**이다. 업무로 만들지 말 것.
(예: 차량의 보험만기일 = 자산 속성 · 지난 정비 내역 = 이력 · 「보험 갱신하기」 = **업무**)

분류 SSOT = `lib/work-taxonomy` — 대분류 6축은 **원장과 같은 말**(자산·계약·자금 + 일정·과태료·기타).
업무는 ③이벤트이고, 이벤트는 결국 «어느 원장에 붙는가»로 갈리기 때문이다.

**⚠ 현재 갭 (미구현):** 지금 `work_item`은 **사람이 손으로 만들 때만** 생긴다.
`buildAgenda`는 어김·임박·예정을 **계산만** 하고 업무를 만들지 않는다 — 검사만기가 닥쳐도
달력에 뜰 뿐 「할 일」이 생기지 않는다. 정의대로라면 **조건이 업무를 낳아야** 한다
(상태=지시 방아쇠). 자동 생성 경로는 미착수 — `RENMAN-CURSOR.md` §2 참조.
**메뉴 SSOT** = `lib/nav` `NAV_GROUPS` / `ERP_MENU_TREE` (역할표 = `PAGE_IA`). 업무 딥링크 = `lib/work-hub` `WORK_PAGES`.

| 그룹 | 항목 | 기준 |
|---|---|---|
| 원장 | 자산 · 계약 · 자금 | ① 마스터 표 + 우측 패널 |
| 업무 | 업무관리 | ③ **할 일이 생성되는 곳**(담당·기한·상태가 붙는 것만) |
