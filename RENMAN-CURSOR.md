...
- **업무 세부 채우기 — 계약 축이 비어 있다.** 대분류 6축은 확정(자산·계약·자금·일정·과태료·기타)이고
  ...
  **미결 판단:** 인도·반납·해지는 계약화면에서 상태를 바꾸는 버튼이 이미 있다. 업무로도 만들면 경로가 둘이 된다.
  ...
  ⚠ **2026-08-07 자동생성이 이 판단을 반쯤 선점했다** — 반납 만기 자동업무를 담으려고 세부 `반납·정산` 을
  신설하면서 **「업무=예정·준비, 전이는 계약화면」 쪽으로 가정**하고 만들었다(폼에 상태전이 버튼 없음).
  **사장님이 반대로 정하시면** `lib/work-form-sections.ts` 의 「반납 준비」 섹션에 실행 버튼을 붙이면 된다.
  나머지 4종(인도·중도해지·연장·계약변경)은 **아직 세부가 없다** — 판단 후 같은 방식으로 추가.
