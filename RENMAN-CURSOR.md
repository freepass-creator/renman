...
- **차량360(`/vehicle/[plate]`)을 상세패널 규격으로.** 지금은 623줄 페이지에 `Sec`+`ObjCard` 카드 나열이라
  규격 밖이다(필드 하나 옮기려면 컴포넌트를 고쳐야 한다). **목표 = 원장 상세패널과 같은 모양** —
  접히는 섹션 + 그 안에 KV 표. **이력(계약·정비·사고·과태료)은 섹션 안 작은 표**(사장님 확정 · 탭 분리 아님).
  360의 뜻 = 「자산 1대에 대한 계약이력·모든 이력을 한 화면에서 본다」.
  ▸ **토대는 놓았다**: `LedgerRecordSection` 에 `body`(섹션 안 표)·`count`(건수 배지) 추가 완료.
    남은 일 = `VEHICLE_DETAIL_DEFS` 정의표 작성 + `VehicleDetail.tsx` 를 그 표 기준으로 재작성.
    고정값(등록증·제원·취득·보험·운영)=KV · 이력=`body` 표.
