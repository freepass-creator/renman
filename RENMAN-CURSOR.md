...
- **차량360(`/vehicle/[plate]`) 규격 이식 — 규격서 = `docs/VEHICLE360-SPEC.md` (그것만 읽고 한 번에).**
  ...
  남은 일 = **이력 6종을 섹션 안 표로 일괄 전환**(계약이력·수납스케줄·할부상환·정비/사고·과태료·손바뀜·활동).
  ⚠ 업로드(등록증·보험증권) 경로를 지우지 말 것 — 오픈 조건 ①의 유일한 상시 화면이다.
- **업무 자동 생성 — 조건이 업무를 낳아야 한다.** 업무관리 정의 = 「할 일이 생성되는 곳」(`CLAUDE.md` 데이터 3층).
  그런데 지금 `work_item`은 **손으로 만들 때만** 생기고 `buildAgenda`는 계산만 한다 — 검사만기가 닥쳐도
  달력에 뜰 뿐 할 일이 안 생긴다. 어젠다 항목(어김·임박·예정)을 업무로 승격하는 경로가 필요하다.
  **주의:** 매번 새로 만들면 중복이 쌓인다 → 근거키(`sourceKey`= 대상+종류+기한) 로 dedup 필수.
- **업무 세부 채우기 — 계약 축이 비어 있다.** 대분류 6축은 확정(자산·계약·자금·일정·과태료·기타)이고
  ...
  **미결 판단:** 인도·반납·해지는 계약화면에서 상태를 바꾸는 버튼이 이미 있다. 업무로도 만들면 경로가 둘이 된다.
  → 「업무=예정·준비(반납예정·해지접수), 실제 전이는 계약화면」으로 가를지, 업무에서 바로 실행할지 사장님 결정 대기.
...
## 4-9. 데이터센터 다음 작업 — **1건 = 파일 N장** (사장님 확정 2026-08-06)

증상 3개(스피너가 엉뚱한 데서 돈다 · 상세패널이 「무슨 문서를 올렸는지」를 못 보여준다 ·
등록증 앞/뒤가 따로 논다)는 **한 가지 원인**이다: 지금은 **1파일 = 1건**이다.

**건이 문서고, 장이 그 문서의 페이지다.**

```
건(문서)  ── 종류: 자동차등록증 · 대상: 15가4481 · 회사
           ├ 1장  reg_front.jpg   ✔ 읽음
           ├ 2장  reg_back.jpg    ⟳ 분석 중      ← 진행표시는 여기, 화면 아님
           └ 3장  reg_note.pdf    ✖ 미인식
```

- `IngestPendingRow`(`lib/ingest-summary.ts`)는 지금 `record` 1개를 문다 → **`pages: {file,status,extracted}[]`** 로.
  `stagedFiles: File[]`(`app/ingest/page.tsx:85`)는 이미 다중이다 — **묶는 단계만 없다.**
- **상세패널 최상단 = 이 장 목록.** 패널 열자마자 «무슨 문서 몇 장»이 보여야 한다. 각 장 열기/삭제.
- **스피너는 그 장의 행 안**에서 돈다. 전면 가림 금지 → 그래야 패널이 안 닫힌다.
- OCR 결과는 **장별 병합**(앞장 번호판 + 뒷장 이력). 충돌 시 `OcrCrosscheck` 로.
- 문서 종류는 §3-2(`docs/VEHICLE360-SPEC.md`)의 **같은 목록**을 쓴다. 두 벌 금지.
- **업로드 버튼은 누르면 「취소」로 바뀐다**(토글 1개). 「업로드」 옆에 「취소」를 나란히 두지 않는다
  — 진행 중에만 멈출 수 있으니 자리도 같아야 한다. 원장 쓰기버튼과 같은 규격.

### 4-9-1. 축 4개 — 「문서 종류」 하나로는 부족하다 (사장님 지적 2026-08-06)

종류(`자동차등록증`)는 **이름표**다. 시스템이 알아야 할 건 **어떻게 처리하냐**고, 그건 종류가 아니라
**성격**이 정한다. 지금 `fileForm` 한 축만 있어서 분류·필터가 다 뭉개진다.

| 축 | 값 | 무엇을 정하나 |
|---|---|---|
| **① 파일형태** (물리) | 엑셀 · PDF · 이미지 · 한글 · 직접입력 | 파서 선택(SheetJS / Gemini / 없음) — 이미 `fileForm` |
| **② 성격** ★신설 | **원장자료 · 증서 · 계약서 · 청구/영수 · 증빙** | **처리 방식**. 아래 표 |
| **③ 귀속** | 자산 · 계약 · 자금 · 과태료 · 기타 | 어느 원장으로 가나 — **`lib/work-taxonomy.ts` WORK_DIVISIONS 재사용**(두 벌 금지) |
| **④ 종류** | 자동차등록증 · 매매계약서 · 세금계산서 … | 사람이 부르는 이름. ②③ 안의 자유 입력 허용 |

**②가 처리 방식을 정한다 — 이게 핵심이다:**

| 성격 | 예 | 처리 |
|---|---|---|
| **원장자료** | 사업현황.xlsx · 자금일보 · 거래내역 | 표를 파싱해 **N건 투입**. OCR 아님 |
| **증서** | 자동차등록증 · 보험증권 · 사업자등록증 | OCR → **기존 1건 갱신** + 정합성 검사(`doc-crosscheck`) |
| **계약서** | 매매 · 렌탈 · 할부 | 조건 추출 + **원본 봉인**(버전관리 필수) |
| **청구/영수** | 세금계산서 · 견적서 · 고지서 | 금액·기한 추출 → 자금/과태료로 |
| **증빙** | 차량사진 · 검사증 · 사고명세 | **추출 안 함**. 첨부만 — 분석상태가 「미인식」으로 뜨면 안 된다 |

- ⑤ **분석상태**(읽음 · 미인식 · 실패 · 해당없음)는 파생값이지 입력 축이 아니다.
  성격=증빙이면 **「해당없음」**이다. 지금 전부 「미인식」으로 뜨는 게 그래서다.
- 필터패널 축 = ① ② ③ ⑤ (④는 검색으로). 앞서 적은 「파일형태·문서종류·카테고리·분석상태」를 이걸로 대체.

### 4-9-2. 목록 열 — 「어디 업무냐 · 어디로 가냐」가 보여야 한다 (사장님 2026-08-06)

분류만 하고 목록에 안 보이면 소용없다. **올린 것을 목록에서 분류해서 볼 수 있어야** 한다.
타이틀헤더는 **자료가 0건이어도 미리 서 있다**(빈 표에도 헤더 유지 — 이미 반영됨).

| 열 | 예 | 비고 |
|---|---|---|
| 형태 | `PDF` | ① |
| 성격 | `증서` | ② — 처리 방식이 여기서 보인다 |
| 귀속 | `자산` | ③ `WORK_DIVISIONS` |
| **대상** | `15가4481` | **어디에 관련된 업무인가.** 없으면 **「미배정」 빨강** |
| **행선지** | `자산원장 › 차량` | **어디로 갈 것인가.** 클릭 = 그 원장으로 딥링크 |
| 장 | `2장` | §4-9 pages[] |
| 분석 | `읽음` / `해당없음` | ⑤ 파생 |
| 올린날 | `08-06` | |

- **행선지는 입력이 아니라 계산값**이다: 성격②+귀속③ → 목적지. (증서+자산 → 자산원장 차량 /
  원장자료+자금 → 자금원장 거래 / 청구영수+과태료 → 과태료원장)
- **대상 미배정 = 빨강**. 그게 실무자가 실제로 처리해야 할 일이고, 지금 상단 통계의
  `미배정` 숫자와 같은 값이다 — 숫자를 누르면 그 행들만 남아야 한다(필터 연동).
- 성격=**증빙**이면 분석 열은 「해당없음」. 회색. 「미인식」 아님.

**이건 조각으로 손대면 또 섞인다.** 위 구조를 한 번에 바꾸고, 필터패널
(파일형태·문서종류·카테고리·분석상태, 원장 4면 `LedgerFilterButton`+`LedgerFilterPanel`)은
장 목록이 선 다음에 얹는다.
