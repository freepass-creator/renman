...
- **OCR 추출 중에도 그 업로드 박스를 유지할 것.** 지금은 추출이 시작되면 박스 자리가 바뀌어
  «내가 올린 게 뭐였는지»를 놓친다. 올린 파일이 보이는 채로 그 안에서 진행표시만 돌아야 한다.
- **데이터센터 목록 = 원장 규격 표 + «무엇을 읽었나» 요약.** 두 가지가 같이 틀렸다.
  ① 몸집 — 대기/저장본 목록이 원장 4면(`ExcelSheet`·행문법·열 SSOT)과 다르다. 박스형 손롤 금지.
  ② **내용** — 지금은 엔티티 필드를 그대로 늘어놓는다. 그게 아니라
     **문서 종류 · 카테고리 · 분석 결과 요약**이 와야 한다.
     실무자가 대기표에서 확인하는 건 «이 종이가 뭐고, 뭘 읽어냈나»지 필드 나열이 아니다.
     예: `자동차등록증 · 차량 · 86버1166 카니발 / VIN·검사만기 읽음` · `신뢰도`·`미인식 항목`.
     상세 필드는 행을 열었을 때(우측 패널) 본다.
  ③ **헤더는 빈 상태에서도 이미 서 있어야 한다.** 지금은 자료가 없으면 표 골격째 사라지고
     안내문만 남는다 — 「무엇을 올리면 뭐가 채워지는지」를 못 본다. 열 헤더는 상시 노출.
  ④ **파일 형태를 판별해 보여줄 것** — 엑셀 / PDF / 이미지(사진·스캔). 확장자·MIME 로 판정.
     그 형태에서 «무슨 서류인지»·«어느 업무 분류인지»를 이어서 보여준다.
     즉 한 행이 답해야 할 것: **무슨 파일인가 → 무슨 서류인가 → 어느 분류(업무/원장)인가 → 뭘 읽었나**.
