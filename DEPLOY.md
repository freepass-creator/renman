...
## ★★ 오픈 범위 — 「어디까지 되면 오픈인가」 (사장님 확정 2026-08-06)

**전부 다 되어야 오픈이 아니다.** 오픈은 아래 **세 경로**가 끝까지 돌면 한다.
이 정의가 없으면 백로그가 남아 있다는 이유로 오픈이 무한히 밀린다.

| # | 경로 | 끝났다고 말할 수 있는 조건 |
|---|---|---|
| ① | **자산 등록** | 차량을 등록하고 **자동차등록증 · 보험 가입증명서(증권)** 를 갖춰 붙일 수 있다 |
| ② | **자금 등록 → 자금일보** | 돈을 넣으면 그날 **자금일보가 생성**되고 분류·마감까지 이어진다 |
| ③ | **운영현황 확인** | ①②의 결과를 `/status` 에서 확인할 수 있다 (보유 차량 1대=1행) |

**오픈 필수 아님:** 업무관리는 필요할 때마다 분류를 잡아 넣으면 된다 —
분류 체계(대분류 5축)는 잡혀 있고 세부는 늘려나가는 구조다(`lib/work-taxonomy`).

**판단 기준:** 「실무자가 그 일을 **끝낼 수 있는가**」. 번거로운 것은 오픈 후 개선,
끝낼 수 없는 것만 오픈 블로커다.
